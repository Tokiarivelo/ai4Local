'use client';

import React from 'react';
import { Card } from '@/app/modules/ui/card';
import { Button } from '@/app/modules/ui/button';
import { Users, Target, Filter, Database } from 'lucide-react';

interface AudienceStepProps {
  isValidating?: boolean;
  onAsyncValidation?: (data: any) => Promise<void>;
  isEditing?: boolean;
}

/**
 * Étape 3: Audience et ciblage
 * Sélection des segments, filtres personnalisés
 */
export function AudienceStep({
  isValidating = false,
  onAsyncValidation,
  isEditing = false,
}: AudienceStepProps) {
  return (
    <div className="space-y-8">
      <Card className="p-6">
        <div className="text-center py-12">
          <Users className="w-16 h-16 text-gray-400 mx-auto mb-4" />
          <h3 className="text-lg font-semibold mb-2">Audience et ciblage</h3>
          <p className="text-muted-foreground mb-6">
            Cette étape sera implémentée prochainement. Elle permettra de :
          </p>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 max-w-2xl mx-auto">
            <div className="flex flex-col items-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <Target className="w-8 h-8 text-blue-500 mb-2" />
              <span className="text-sm">Segments</span>
            </div>
            <div className="flex flex-col items-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <Filter className="w-8 h-8 text-green-500 mb-2" />
              <span className="text-sm">Filtres</span>
            </div>
            <div className="flex flex-col items-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <Database className="w-8 h-8 text-purple-500 mb-2" />
              <span className="text-sm">Import CSV</span>
            </div>
            <div className="flex flex-col items-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <Users className="w-8 h-8 text-orange-500 mb-2" />
              <span className="text-sm">Estimation</span>
            </div>
          </div>
          <Button className="mt-6" disabled>
            Fonctionnalité en développement
          </Button>
        </div>
      </Card>
    </div>
  );
}
